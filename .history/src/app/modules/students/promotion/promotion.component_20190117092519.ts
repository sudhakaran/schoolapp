...
export class PromotionComponent implements OnInit {
  ...
  public promotionStatusList = [
    { label: 'Active', value: 'Active' },
    { label: 'Promoted', value: 'Promoted' },
    { label: 'Got TC', value: 'Got TC' }
  ];
  ...
  public getStudentsPromotionList(searchStudent) {
    if (!this.searchStudentPromotionForm.invalid) {
      this._studentService
        .studentsPromotionList(searchStudent)
        .then(response => (this.studentsListForPromotion = response));
      this.cols = [
  ...
  public savePromotionsList(promotedStudentsList) {
    this.confirmationService.confirm({
      message: 'Are you sure to save this action?',
      header: 'Save Promotion Confirmation',
  ...
  public clearSearchResults() {
    this.confirmationService.confirm({
      message: 'Are you sure to cancel this action?',
      header: 'Cancel Promotion Confirmation',
      icon: 'pi pi-info-circle',
      accept: () => {
        // this.msgs = [{ severity: 'info', summary: 'Confirmed', detail: 'Record deleted' }];
