...
export class LoginService {
  ...
  public login(userDetails): Observable<object> {
    ...
    return this.http.post(`http://148.72.211.55:8080/schoolapp/auth/login`, loginUSerDetails, httpOptions)
      .pipe(
        map(userToken => {
  ...
  private handleError(error: Response) {
    return Observable.throw(error || 'An error occurred while communicating with the API');
  }
