...
export class AdmissionService {
  ...
  public getAdmissionByYearStd(admissionDetails: any): Observable<any> {
    const url = `${this._baseApiUrl}/studentsmanagement/getadmissionbyyearstd?acedamicYear=${admissionDetails.acedamicyear}&standard=${admissionDetails.standard}%20Std&branch=${admissionDetails.applicationNo}`;
    const options = this._getRequestOption();
    return this.http.get(url, options)
      .pipe(
        map(data => data),
