...
export class AdmissionService {
  ...
  public getAdmissionByAppNo(applicationNo: any): Observable<any> {
    const url = `${this._baseApiUrl}/studentsmanagement/getadmisisonbyappno?applicationNo=AN123456`;
  ...
  public getAdmissionByYearStd(admissionDetails: any): Observable<any> {
    const url = `${this._baseApiUrl}/studentsmanagement/getadmissionbyyearstd?acedamicYear=${admissionDetails.acedamicyear}&standard=${admissionDetails.standard} Std&branch=${admissionDetails.applicationNo}`;
    const options = this._getRequestOption();
    return this.http.get(url, options)
      .pipe(
        map(data => data),
